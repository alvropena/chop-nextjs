...
export default function AsideMenu({ }) {
  ...
  const baseClasses =
    "relative inline-flex w-full items-center text-sm px-3 font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 h-9 rounded-md";
          ...
          <DialogTrigger asChild>
            <button className={searchButtonClasses}>
              <SearchIcon className="h-5 w-5 shrink-0" />
               <span className={cn("ml-2", isCollapsed && 'hidden')}>{t("search")}</span>
            </button>
          </DialogTrigger>
          <DialogContent>
            <SearchScreen />
