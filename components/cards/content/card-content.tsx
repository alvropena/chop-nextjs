...
import { CardContentType } from '../../../types/card/content/card-content-type';

const CardContent: React.FC<CardContentType> = ({ passage, audioUrl, imageUrl }) => {
  return (
    <div>
      ...
      {imageUrl && (
        <div className="mt-4 w-full relative overflow-hidden rounded" style={{ height: "200px" }}>
          <Image src={imageUrl} alt="Content image" layout="fill" objectFit="cover" />
        </div>
      )}
    </div>
  );
